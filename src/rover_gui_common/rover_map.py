...
from urc_gui_common import MapPoint, MapTab, HLine
from urc_gui_common.helpers.file_helper import file_basenames
from urc_gui_common.helpers.validator import red_if_unacceptable, valid_entries

from rover_gui_lib.msg import EDWaypoint, EDWaypointList

from rover_gui_common.ui_python.ed_edit_dialog import ExtremeDeliveryEditWaypointDialog
from rover_gui_common.rover_ros_link import RoverRosLink
...
class RoverMapTab(MapTab):
	...
	def init_markers_tab(self):
		...
		waypoints_tab.insertTab(1, self.ed_waypoint_input, "ED")
	...
	def insert_ed_waypoint_handler(self):
		...
		try:
			...
			self.ed_waypoints.insert(new_following_waypoint_id, EDWaypoint(latf, lonf, radf, name))
			self.ed_publish_waypoints()
	...
	def add_ed_waypoint_handler(self):
		...
		try:
			...
			self.autosave_enabled = True
			self.ed_waypoints.append(EDWaypoint(latf, lonf, radf, name))
			self.ed_publish_waypoints()
